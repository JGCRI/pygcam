...
def setupWorkspace(workspace):
    ...
    if not os.path.isdir(workspace):
        _logger.info("Creating GCAM workspace '%s'", workspace)
    ...
    # For basex
    if os.path.lexists(os.path.join(gcamWorkspace, 'exe', 'WriteLocalBaseXDB.class')):
        workspaceSymlink('exe/WriteLocalBaseXDB.class')

    # Add symlinks to dirs holding files generated by "setup" scripts
    def xmlLink(varName, xmlDir):
        src = getParam(varName)
        dst = os.path.join(workspace, xmlDir)
