...
class XMLEditor(object):
    ...
    def __init__(self, mapper, cleanXML=True):
        ...
        self.mapper = mapper
        self.name = scenario = mapper.scenario
        self.setupArgs = None
        ...
        # self.solution_prefix_abs = pathjoin(mapper.ref_workspace, "input", "solution")
        ...
        if cleanXML and scenario and mapper.mcs_mode != McsMode.TRIAL:
            with pushd(self.scenario_dir.abs):
                files = glob.glob('*')
                if files:
                    _logger.debug("Deleting old files from %s: %s", self.scenario_dir.abs, files)
                    for name in files:
    ...
    def setupDynamic(self, args):
        ...
        dynDir = mapper.sandbox_dynamic_xml
        _logger.debug(f"Removing tree '{dynDir}'")
        ...
        #   Candidate for deletion once dynamic stuff is reconsidered.
        if xmlFiles:
    ...
    def protectionScenario(self, scenarioName, unprotectFirst=True):
        ...
        **Callable from XML setup files.**
        ...
        if self.mcsMode == McsMode.TRIAL:
            ...
            # TBD: fix this pathjoin() to use self.mapper instead
