...
In :doc:`tutorial1`, we created a new project called "ctax",
examined some of its characteristics, and run a few scenarios.

Here, in Part 2, we will examine the output files and customize
some of the generated charts.


2.1 Examine output files
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
2.2 Run policy cases for default scenario group
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
2.3 Examine policy case results
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
2.7 Run on a computing cluster
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
